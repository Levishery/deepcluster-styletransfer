...
def cluster_assign(images_lists, dataset, PATCH_NUM):
    ...
    img_num = len(dataset)
    total_patch = PATCH_NUM*PATCH_NUM
    pseudolabels = np.zeros((img_num, total_patch))
    image_indexes = range(0, img_num)
    for cluster, images in enumerate(images_lists):
        for img in images:
            pseudolabels[img//total_patch][img%total_patch] = cluster
...
class PIC():
    ...
    def cluster(self, data, verbose=False):
        ...
        # preprocess the data
        xb = preprocess_features(data, 64)
